...
class PrompterEditor(wx.stc.StyledTextCtrl):
    def __init__(self, parent, config, suggestions):
        ...
        class PrompterPresageCallback(convAssist.callback.Callback):
        ...
        callback = PrompterPresageCallback(self)

        if config:
            #self.prsg = presage.Presage(callback, config)
            self.prsg = convAssist.ConvAssist(callback, config)
        else:
            #self.prsg = presage.Presage(callback)
            self.prsg = convAssist.ConvAssist(callback,None)
        self.prsg.cannedPhrase_recreateDB()
        if suggestions:
        ...
        self.AutoCompSetMaxHeight(int(self.prsg.config.get("Selector", "suggestions")))
        #int(config.get("Selector", "suggestions"))
        self.text_size = 10
    ...
    def __ShowPrediction(self, string = ''):
        ...
        print("CONTEXT = ", self.prsg.context_tracker.past_stream())
        try:
            #prefix = self.prsg.prefix()
            prefix = self.prsg.context_tracker.prefix()
            #context = self.prsg.context()
            context = self.prsg.context_tracker.past_stream()
            ...
            #nextLetterProb, preds = self.prsg.predict()
            print("Word_nextLetterProb = ", wordprob)
            print("NextWord_predictions = ", words)
            print("Sent_nextLetterProb = ", sentprob)
            print("Sent_completions = ", sent)
            predlist = []
            for p in words:
                predlist.append(p[0])
            for p in sent:
                predlist.append(p[0])
            self.prediction = predlist
            # added to see if learning works
            # learn should happen after happens
            # if random.randint(0, 9) <10:  #is this a good idea to update occasionally?
            #     self.prsg.context_tracker.update_context()
            ...
            print("SELF.SUGGESTIONS = ", self.suggestions)
    ...
    def OnUserListSelection(self, event):
        ...
        #completion = str(event.GetText(), 'utf-8')
        completion = str(event.GetText())
        if self.function_keys_enabled:
            completion = self.__RemoveFunctionLabel(completion)
        #prefix_length = len(unicode(self.prsg.prefix()))
        #prefix_length = len(str(self.prsg.prefix(), 'utf-8'))
        prefix_length = len(str(self.prsg.context_tracker.prefix()))
